...
export default function App() {
  ...
  // Load backend profiles on mount and subscribe to real-time updates (Only ON MOUNT)
  useEffect(() => {
    setIsClient(true);
    ...
    // Subscribe to Firestore profiles in real-time
    const unsubscribe = subscribeToProfiles(
      (data) => {
        setProfiles(data);
        
        // Resolve active profile deletion logic
        const activeId = savedActiveId || currentProfileId;
      ...
      (err) => {
        ...
        setLoadingError("실시간 랭킹 데이터베이스와의 연결이 원활하지 않습니다. 잠시만 대기해주세요.");
      }
    );
  ...
  const handleLogin = async (grade: string, classGroup: string, number: string, passwordInput: string): Promise<{ success: boolean; error?: string }> => {
    try {
      const res = await loginStudent(grade, classGroup, number, passwordInput);
      if (!res.success || !res.profile) {
        return { success: false, error: res.error || "로그인 정보를 확인해주세요." };
      }
      
      setCurrentProfileId(res.profile.id);
      localStorage.setItem(ACTIVE_PROFILE_ID_KEY, res.profile.id);
      return { success: true };
    } catch (error: any) {
  ...
  const handleSignUp = async (signUpData: any): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (!res.success || !res.profile) {
        return { success: false, error: res.error || "회원가입에 실패했습니다." };
      }
  ...
  const handleSaveUserInfo = async (newInfo: UserInfo) => {
    ...
    try {
      await updateStudentInfo(currentProfileId, newInfo);
      setIsEditModalOpen(false);
    } catch (error) {
      console.error(error);
      alert("프로필 정보를 수정하는 도중 오류가 발생하였습니다.");
    }
  };
  ...
  // Update entries of currently active profile in Firestore
  const handleUpdateActiveEntries = async (
    ...
    try {
      const mappedEntries: ScreenTimeEntry[] = newEntries.map((entry, index) => {
      ...
      await updateStudentEntries(currentProfileId, mappedEntries);
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="min-h-screen bg-[#17171B] text-[#F3F4F6] flex flex-col font-sans selection:bg-emerald-500/30 selection:text-white" id="screen-time-app">
      ...
      <footer className="w-full border-t border-neutral-800 bg-neutral-950/20 py-4.5 text-center text-xs text-neutral-500 font-sans mt-auto">
        <p>© 2026 ScreenTime Tracker. 한국 학급 디지털 디톡스 캠페인 • 모든 데이터는 실시간 클라우드 랭킹 데이터베이스에 안전하게 공유됩니다.</p>
      </footer>
    </div>
  );
}
